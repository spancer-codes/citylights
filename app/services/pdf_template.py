...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── Scope of work (no-pricing mode) ── */
  .scope-box { margin-bottom: 16px; }
  .scope-label {
    font-size: 10px; font-weight: 700; color: #8a92a6;
  ...
  .scope-list li::before {
    content: "•"; color: #0D7377; font-weight: 700;
    position: absolute; left: 8px;
  }
  .scope-list li:nth-child(odd)  { background: #eef1f8; }
  .scope-list li:nth-child(even) { background: #dce2f2; }
  .scope-note { font-size: 10px; color: #6b7385; margin-top: 2px; }
  ...
  .approval-text {
    font-size: 11px; color: #5a6275; line-height: 1.5;
    margin-bottom: 22px;
  }
  .approval-grid { display: flex; gap: 24px; }
  .approval-field { flex: 1; }
  .sig-line { border-bottom: 1px solid #1a1a2e; height: 30px; }
  .sig-label {
  ...
  .approval-note {
    font-size: 10px; color: #8a92a6; margin-top: 14px;
  }
...
<div class="page">
  ...
  <div class="body">
    ...
    <table class="items-table">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td class="r" style="font-weight:700">R{{ "%.2f"|format(item.total_price) }}</td>
        </tr>
        {% endfor %}
      </tbody>
    </table>
    ...
    {% else %}
    <!-- Scope of Work (labour-only, no item pricing shown) -->
    <div class="scope-box">
      ...
      <ul class="scope-list">
        ...
        <li>
          {{ item.description }}
          {% if item.get('note') %}<div class="scope-note">{{ item.note }}</div>{% endif %}
        </li>
        {% endfor %}
      </ul>
    </div>
    ...
    <div class="totals-wrap">
      <div class="totals-box">
        <div class="totals-row grand"><span>Total Labour</span><span class="amt">R{{ "%.2f"|format(total_labour) }}</span></div>
        {% if deposit_percent %}
        <div class="totals-row deposit">
          <span>{{ deposit_percent }}% Deposit required</span>
          <span>R{{ "%.2f"|format(total_labour * deposit_percent / 100) }}</span>
        </div>
        {% endif %}
      </div>
    </div>
    ...
    <div class="approval-box">
      <div class="approval-label">Client Approval</div>
      <div class="approval-text">
        By signing below, I confirm that I have reviewed and accept this quotation in full, and I authorise
        City Lights Electrical Services to proceed with the work described above.
      </div>
      ...
      <div class="approval-note">
        Work will commence only once a signed copy of this quotation has been received.
      </div>
    </div>
